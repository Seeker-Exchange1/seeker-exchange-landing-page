    ...
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="text-left">
        ...
        <select
          ...
          required
          className="w-full bg-white/5 border border-white/20 text-white h-12 px-4 rounded-lg focus:ring-2 focus:ring-[#c8ff00]/50 focus:outline-none"
          disabled={status === "loading"}
        >
          {countries.map((c) => (
      ...
      <Button
        ...
        className="w-full bg-[#c8ff00] text-black hover:bg-[#b8ef00] font-semibold h-12 rounded-lg text-lg disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {status === "loading" ? "Submitting..." : "Join Waitlist"}
      </Button>

      {message && (
        <p className={`mt-4 text-center ${status === "success" ? "text-[#c8ff00]" : "text-red-400"}`}>{message}</p>
      )}
    </form>
  )
}
